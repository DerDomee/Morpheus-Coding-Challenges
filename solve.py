...
def prompt_yes_no(question, default=None):
    ...
    while True:
        ...
        if default is not None and choice == '':
            ...
            sys.stderr.write("Please respond with 'Y' or 'N' only. " +
                             "(Or with yes/no)\n")
...
def parse_arguments():
    ...
    def print_unknown_argument():
        sys.stderr.write("Error parsing argument '" + argument_instance +
                         "' at index " + str(i + 1) + ": Unknown argument.\n")
        arg_errored = True

    def print_double_argument():
        sys.stderr.write("Warning parsing argument '" + argument_instance +
                         "' at index " + str(i + 1) +
    ...
    def print_illegal_help_argument():
        sys.stderr.write("Error parsing argument '--help' at index " +
...
def solve_challenges(run_args):
    ...
    solvefile = None
    try:
        solvefile = importlib.import_module('challenges.chall' +
                                            str(run_args['challenge_number']))
...
def parse_result(run_args, run_result):
    ...
    for i in range(run_args['runs_number']):
        ...
        if timing_data[key]['total_delta'] > \
           ...
           statistics['worst_inst_totaldelta'] = timing_data[key]['total_delta']

        if timing_data[key]['total_delta'] < \
            statistics['best_inst_totaldelta']:
            statistics['best_inst_totaldelta'] = timing_data[key]['total_delta']
